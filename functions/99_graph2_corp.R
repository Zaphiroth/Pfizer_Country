  ...
  function(salesdata,
    ...
    if (!is.null(level) && level %in% c("corporation", "molecule")) {
      if (level == "corporation") {
        ...
        salesdata5 <-
          transform(salesdata5, prod.sum = ave(
            value,
            paste(AUDIT.DESC,
                  # level1,
                  CORPORATE.DESC,
        ...
        salesdata5 <-
          salesdata5[!duplicated(salesdata5[, c("AUDIT.DESC", 
                                                level1,
                                                # "CORPORATE.DESC", 
                                                # "PRODUCT.DESC",
                                                "date")]),]
        salesdata5$value <- salesdata5$prod.sum
        
        salesdata5 <-
          salesdata5[order(salesdata5[, "AUDIT.DESC"], 
                           salesdata5[, level1], 
                           # salesdata5[, "CORPORATE.DESC"], 
                           # salesdata5[, "PRODUCT.DESC"],
                           salesdata5[, "date"]),]
        ...
        if ("mc" %in% kpi) {
          if (!"ms" %in% names(salesdata5)) {
            salesdata5 <-
              transform(salesdata5, ms = ave(value, paste(AUDIT.DESC,   date), FUN = calms))
          }
          
          salesdata5 <-
            transform(salesdata5, mc = ave(
              ms,
              paste(AUDIT.DESC,
                    CORPORATE.DESC,
                    # PRODUCT.DESC,
          ...
          salesdata5$mc <- round(salesdata5$mc, 2)
        }
        
        if ("pi" %in% kpi) {
          chinaref <-
            salesdata5[which(salesdata5$AUDIT.DESC == "China"),
                       c(#"CORPORATE.DESC",
                         #"PRODUCT.DESC",
                         level1,
                         "date",
          ...
          salesdata5 <-
            transform(salesdata5, sum.market = ave(value, paste(AUDIT.DESC,  date), FUN = sum.nona))
          chinaref.cate <-
            salesdata5[which(salesdata5$AUDIT.DESC == "China"), 
                       c(#"CORPORATE.DESC",
                         #"PRODUCT.DESC",
          ...
          salesdata5$pi <-
            100 * salesdata5$value / salesdata5$cn.value / salesdata5$sum.market *
            salesdata5$cn.sum.market
          salesdata5$pi <- round(salesdata5$pi, 2)
        }
        
        if ("ev" %in% kpi) {
          if (!"gr" %in% names(salesdata5)) {
            salesdata5 <-
              transform(salesdata5, gr = ave(
                value,
                paste(AUDIT.DESC,
          ...
          if (!"sum.market" %in% names(salesdata5)) {
            salesdata5 <-
              transform(salesdata5, 
          ...
          salesdata5 <-
            transform(salesdata5, gr.market = ave(
              sum.market,
              paste(AUDIT.DESC, 
                    CORPORATE.DESC,
                    # PRODUCT.DESC, 
          ...
          salesdata5 <-
            transform(salesdata5, ev = 100 * (gr + 100) / (gr.market + 100))
          salesdata5$ev <- round(salesdata5$ev, 2)
        }
        
        if ("abs" %in% kpi) {
          salesdata5$abs <- salesdata5$value
          salesdata5$abs <- (round(salesdata5$abs, 0))
        }
        
        if ("ms" %in% kpi) {
          salesdata5$ms <- round(salesdata5$ms, 2)
        }
        
        if ("gr" %in% kpi) {
          salesdata5$gr <- round(salesdata5$gr, 2)
        }
        # salesdata5$CORPORATE.DESC <- "Overall"
        
        salesdata5$PRODUCT.DESC <- "Overall"
        salesdata5$Category <- "Overall"
        salesdata5$Sub.category <- "Overall"
        
        salesdata5$COMPS.DESC <- "Overall"
        
        salesdata5$pro.code <- "Overall"
        
        salesdata5$MANUF.TYPE.DESC <- "Overall"
        salesdata5$TC.III.SHORT.DESC <- "Overall"
        salesdata5$TC.III.DESC <- "Overall"
        
        salesdata5$Category_CN <- "Overall"
        salesdata5$Molecule_CN <- "Overall"
        salesdata5$Brand_CN <- "Overall"
        salesdata5$MANU_CN <- "Overall"
      ...
      maindata <- rbind.fill(maindata, salesdata5)
    }
